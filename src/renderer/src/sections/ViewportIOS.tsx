...
export const ViewportIOS: Component<{
	...
	createEffect(() => {
		if (projectFrame.state.mode === "dark") {
			setStatusBarColor("white");
		} else {
			if (projectFrame.state.open && projectFrame.state.expanded) {
				setStatusBarColor("white");
			} else {
							...
							>
								<div>
									<span
										...
										onClick={() => setMenuMore("closeOrBack", "clicked", true)}
									>
										{projectInner.backButton.enabled ? "Back" : "Close"}
									</span>
								</div>
								<div>
									<h2>{props.project.name}</h2>
