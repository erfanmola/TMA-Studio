...
export const createWindowProjectFloating = (project: Project['id'], platform: TelegramPlatform) => {
    if (!(project in ipcMainPreferences.windows.popups)) {
        ipcMainPreferences.windows.popups[project] = [];
    }

    if (ipcMainPreferences.windows.popups[project].find(item => item.platform === platform)) return;
    ...
    window.on('closed', () => {
        ipcMainPreferences.windows.main?.webContents.send(`sync-project-${project}-${platform}`);
        ipcMainPreferences.windows.popups[project] = ipcMainPreferences.windows.popups[project].filter((win) => !win.isDestroyed());
    });
