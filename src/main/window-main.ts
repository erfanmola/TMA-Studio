...
export const createWindowMain = (): void => {
    ipcMainPreferences.windows.main = new BrowserWindow({
        ...
        titleBarOverlay: {
            color: ipcMainPreferences.theme.window_widgets[ipcMainPreferences.theme.mode].bg,
            symbolColor: ipcMainPreferences.theme.window_widgets[ipcMainPreferences.theme.mode].color,
            height: 32
        }
    });
    ...
    ipcMainPreferences.windows.main.webContents.on('before-input-event', (event, input) => {
        if (
            ...
            if (!allowedKeys.includes(input.key)) {
                ...
                ipcMainPreferences.windows.main.webContents.send('shortcut-pressed', input);
            }
        }
    });

    ipcMainPreferences.windows.main.webContents.on('will-attach-webview', (_, webPreferences) => {
        webPreferences.preload = join(__dirname, '../preload/webview.js');
    });

    ipcMainPreferences.windows.main.on('ready-to-show', () => {
        if (!ipcMainPreferences.windows.main) return;
        ipcMainPreferences.windows.main.webContents.setZoomFactor(ipcMainPreferences.zoom.level);
        ipcMainPreferences.windows.main.show();
    })

    ipcMainPreferences.windows.main.webContents.setWindowOpenHandler((details) => {
    ...
    ipcMainPreferences.windows.main.on('close', (e) => {
        ...
        if (BrowserWindow.getAllWindows().length > 1) {
            ...
            ipcMainPreferences.tray = new Tray(join(__dirname, '../../icons/tray.png'));
    ...
    if (is.dev && process.env.ELECTRON_RENDERER_URL) {
        ...
        ipcMainPreferences.windows.main.loadFile(join(__dirname, '../renderer/index.html'))
    }
}