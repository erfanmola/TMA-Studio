...
const initializeIPCMisc = () => {
    ...
    ipcMain.on('update-available', async (_, version) => {
        if (ipcMainPreferences.windows.main) {
            dialog.showMessageBox(ipcMainPreferences.windows.main, {
...
const initializeIPCProject = () => {
    ...
    ipcMain.on('project-close', async (_, project, _platform, popup) => {
        if (_.sender.id !== ipcMainPreferences.windows.main?.webContents.id) {
            ...
            const windowItem = ipcMainPreferences.windows.popups[project].find(item => item.webContents.id === _.sender.id);
            if (windowItem) {
                ipcMainPreferences.windows.popups[project].splice(ipcMainPreferences.windows.popups[project].indexOf(windowItem), 1);
            }
        } else if (_.sender.id === ipcMainPreferences.windows.main?.webContents.id && popup) {
...
const initializeIPCSettings = () => {
    ipcMain.on('ui-scale-changed', async (_, ui_scale) => {
        ...
        if (ipcMainPreferences.windows.main && !(ipcMainPreferences.windows.main.isDestroyed())) {
