...
export const createWindowWelcome = (): void => {
	...
	ipcMainPreferences.windows.welcome.webContents.setWindowOpenHandler(
		(details) => {
	...
	if (is.dev && process.env.ELECTRON_RENDERER_URL) {
		ipcMainPreferences.windows.welcome.loadURL(
			process.env.ELECTRON_RENDERER_URL,
		);
	} else {
		ipcMainPreferences.windows.welcome.loadFile(
